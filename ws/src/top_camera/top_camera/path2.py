...
class Path_Planner:
    ...
    def __init__(self,
                 ...
                 collect1=np.array([-9.19407895, -4.55592105]),
                 collect2=np.array([9.14473684, 4.6875]),
                 path=[]):
        self.click1 = click1
        self.click2 = click2
        self.i = i
...
# def distance(self.)
    ...
    def path_planner(self, x, y):
        ...
        if np.sign(self.pixel_to_xy(x, y)[0]) == np.sign(
                self.pixel_to_xy(x0, y0)[0]):
            self.path.append(self.pixel_to_xy(x, y).tolist())
        else:
            path_orig = np.array(self.path)
            path_new = np.array(
                [self.collect1,
                 self.pixel_to_xy(x, y), self.passage1])

            self.path = np.concatenate((path_new, path_orig), axis=0).tolist()
        for i in range(len(self.path) - 1):
            cv2.line(
                self.img, self.xy_to_pixel(self.path[i][0], self.path[i][1]),
                self.xy_to_pixel(self.path[i + 1][0], self.path[i + 1][1]),
                (255, 0, 0), 2)
        print(self.path)

    ...
    def find_match(self, ball, ball_centers):
        ...
        if len(ball_centers) == 0:
            ...
            for i in range(len(ball_centers)):
                n = np.sqrt((ball_centers[i][0] - ball.position[0])**2 +
                            (ball_centers[i][1] - ball.position[1])**2)
                if n < min:
                    min = n
                    closest_ball = i
            return closest_ball

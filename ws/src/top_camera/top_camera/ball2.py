...
class Path_Planner :
    ...
    def path_planner(self,x,y):
        ...
        if x0!=x and np.sign(self.pos[0])==np.sign(self.path[self.ind][0]) :
            self.ind+=1
            self.path=np.insert(self.path,self.ind,self.pos.tolist(),axis=0)
        else :
            self.ind=2
            self.path=np.concatenate((np.array([self.collect_no,self.pos.tolist(),passage]),self.path),axis=0).tolist()
